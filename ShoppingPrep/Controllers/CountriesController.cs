...
using ShoppingPrep.Data;
using ShoppingPrep.Data.Entities;
using ShoppingPrep.Models;

namespace ShoppingPrep.Controllers
{
    public class CountriesController : Controller
    {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(int id, [Bind("Id,Name,States")] Country country)
        {
            if (id != country.Id)
            {
                return NotFound();
        ...
        public async Task<IActionResult> AddCity([Bind("Id,Name,StateId")] CityViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                catch (DbUpdateException dbUpdateException)
                {
                    ...
                    if (dbUpdateException.InnerException.Message.Contains("duplicate"))
                    {
                        ModelState.AddModelError(string.Empty, "Já existe uma cidade com o mesmo nome.");
                    }
                    else
                    {
                        ModelState.AddModelError(string.Empty, dbUpdateException.InnerException.Message);
